                             ...
                             post_processing &pp, Solution &residual, double *local_delta_t, T *local_fneq)
{
   ...
   //enum fileType_e { FULL = 0, GRID = 1, SOLUTION = 2 };
     ...
     double *nodx, *nody, *z, *p,*u, *v,*y ,*x, *u_err, *u_exact ,*w, *vort, *st, *res_rho, *res_u,*res_v, *res_w, *dt, *fneq;
    int *connectivity;
    double solTime;
    INTEGER4 debug, i, j, k, dIsDouble, vIsDouble, zoneType,  parentZn, isBlock;
    INTEGER4 iCellMax, jCellMax, kCellMax, nFConns, fNMode, shrConn, fileType;
    ...
    i = TECAUXSTR142("Re" ,  reynolds_text.c_str());
		...
		fneq = (double*)calloc(nCells, sizeof(double));
        ...
        for (i = 0; i < Mesh.get_n_cells(); ++i){
            ...
            x[t] = (double) Mesh.get_centroid_x(i);
            y[t] = (double) Mesh.get_centroid_y(i);
            z[t] = (double) Mesh.get_centroid_z(i);
            vort[t] = (double) pp.vorticity[i];
            st[t] = (double) pp.streamfunction[i];
...
template<typename T>
void tecplot_output<T>::tecplot_output_lagrangian_object(lagrangian_object &object, global_variables &globals, domain_geometry &geometry, double timestamp)
{
	//ctor
	...
	output_location = globals.output_file + "/plt/object/" + object.name + tt.str() + ".plt";
...
template<typename T>
void tecplot_output<T>::tecplot_output_lagrangian_object_gpu(double * obj_vel_x, double * obj_vel_y, double * obj_vel_z, double * obj_x, double * obj_y, double * obj_z,
